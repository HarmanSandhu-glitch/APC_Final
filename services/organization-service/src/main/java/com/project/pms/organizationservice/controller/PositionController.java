package com.project.pms.organizationservice.controller;

import com.project.pms.organizationservice.entity.Position;
import com.project.pms.organizationservice.service.PositionService;
...
public class PositionController {
  ...
  updatePosition(@PathVariable Long id,
                 @Valid @RequestBody Position positionDetails) {
    return positionService
        .getPositionById(id)
        .map(position -> {
          position.setPositionTitle(positionDetails.getPositionTitle());
          position.setPositionExperienceBonus(
              positionDetails.getPositionExperienceBonus());
          position.setPositionBaseSalary(
              positionDetails.getPositionBaseSalary());
